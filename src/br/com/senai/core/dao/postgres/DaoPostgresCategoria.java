...
public class DaoPostgresCategoria implements DaoCategoria {
	...
	private final String UPDATE = "UPDATE categorias c SET nome = ? WHERE c.id = ?";
	private final String DELETE = "DELETE FROM categorias c WHERE c.id = ?";
	...
	private final String SELECT_TUDO = "SELECT c.id, c.nome FROM categorias c ORDER BY c.nome";
	...
	public ArrayList<Categoria> listarTudo() {
		...
		try {
			ps = conexao.prepareStatement(SELECT_TUDO);
			rs = ps.executeQuery();
			while (rs.next()) {
				categorias.add(extrairDo(rs));
			}
			...
			throw new RuntimeException("Ocorre um erro ao listar categorias no combo box. Motivo: " + e.getMessage());
		} finally {
			ManagerDb.getInstance().fechar(ps);
			ManagerDb.getInstance().fechar(rs);
		}
